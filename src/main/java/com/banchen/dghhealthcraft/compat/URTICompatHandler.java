...
public class URTICompatHandler {
    ...
    private static float getInfectionValue(Player player) {
        ...
        return HealthCapability.getAndApply(player, health -> {
            ...
            if (blood == null || !blood.getBodyConditions().contains(URTI)) 
                return 0f;
            return blood.getConditionValue(URTI);
        }, 0f);
    }
    ...
    public static int getInfectionStage(Player player) {
        ...
        if (value < 0.4f) return 1;
        if (value < 0.7f) return 2;
        return 3;
    }
